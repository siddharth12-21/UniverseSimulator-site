/**
 * Earth globe at origin with clouds and atmosphere.
 */
...
const cloudTex = makePlanetTexture(2048, 1024, (ctx, w, h) => {
  ...
  for (let i = 0; i < 40; i++) {
    const y =
      Math.random() < 0.5 ? Math.random() * h * 0.15 : h - Math.random() * h * 0.15;
...
loader.load(
  ...
  undefined,
  () => {
    earthMat.map = makePlanetTexture(512, 256, texEarth);
    earthMat.needsUpdate = true;
  }
);

export { earth, earthHit, earthClouds, earthMat };
